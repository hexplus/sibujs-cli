...
describe("templates dependency versions", () => {
  ...
  it("sibujs dependency is pinned to ^4.3.0", () => {
    expect(pkgTpl).toMatch(/"sibujs":\s*"\^4\.3\.0"/);
  });

  it("sibujs-ui placeholder resolves to ^1.5.3", () => {
    ...
    expect(createSrc).toMatch(/"sibujs-ui":\s*"\^1\.5\.3"/);
  });

  it("declares a Node floor that satisfies both sibujs 4.0 and vite 8", () => {
    // sibujs 4.3.0 needs >=22.3.0 (process.getBuiltinModule for SSR request
  ...
  it("pins sibujs-cli to this package's own version", () => {
    ...
    expect(pkgTpl).toContain(`"sibujs-cli": "^${own}"`);
  });
});
