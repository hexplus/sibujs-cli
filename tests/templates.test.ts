...
describe("templates dependency versions", () => {
  ...
  it("sibujs dependency is pinned to ^3.2.0", () => {
    expect(pkgTpl).toMatch(/"sibujs":\s*"\^3\.2\.0"/);
  });

  it("sibujs-ui placeholder resolves to ^1.4.1", () => {
    ...
    expect(createSrc).toMatch(/"sibujs-ui":\s*"\^1\.4\.1"/);
  });
});
